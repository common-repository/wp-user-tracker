...
/**
 * Plugin Name: WP User Tracker Pro
	...
	class pantherius_wpusertracker
	{
		...
		**/
		...
		function ajax_wpusertracker_update() 
		{
			...
			{
			$noti_dir = 'right';
			if (get_option('setting_direction_notification')=='left') $noti_dir = 'left';
			$noti_pos = 'top';
			if (get_option('setting_position_notification')=='bottom') $noti_pos = 'bottom';
			$noti_fadeout_time = 1000*3;
			if (get_option('setting_fadeout_time_notification')>0 AND get_option('setting_fadeout_time_notification')!=5) $noti_fadeout_time = 1000*get_option('setting_fadeout_time_notification');
			$donot_disturb_user = 1000*60*5;
			if (get_option('setting_redisplay_notification')>0 AND get_option('setting_redisplay_notification')!=5) $donot_disturb_user = 1000*60*get_option('setting_redisplay_notification');
			$noti_options = base64_encode($noti_dir.'|'.$noti_fadeout_time.'|'.$donot_disturb_user);
				if (isset($_COOKIE['wpusertracker'])) $otherview = $this->wpdb->get_row("SELECT wp.id,wp.post_title FROM ".$this->table_names['users']." us LEFT JOIN ".$this->wpdb->prefix."posts wp on us.last_visited_post=wp.id WHERE `last_visited_post`!= '0' AND us.`id`!='".base64_decode($_COOKIE['wpusertracker'])."' AND NOW() <= DATE_ADD(last_visited_time, INTERVAL 5 MINUTE) ORDER BY last_visited_time DESC");
				if (!empty($otherview)) 
				{
					$permalink = get_permalink($otherview->id);
					if (!get_option('setting_text_notification')) $notification_text = 'someone had just started to read:';
					else $notification_text = get_option('setting_text_notification');
					if (get_option('setting_style_notification')==1) $slidediv = base64_encode(str_replace('{permalink}',$permalink,str_replace('{post_title}',$otherview->post_title,str_replace('{position}',$noti_pos,str_replace('[','<',str_replace(']','>',stripslashes(get_option('setting_style_notification_custom'))))))));
					else $slidediv = base64_encode('<div onclick="document.location=\''.$permalink.'\'" id="wpusertracker_notification" class="wpusertracker_notification_'.$noti_pos.' wpusertracker_notification_default">'.$notification_text.'<br /><center><a href="'.$permalink.'">'.$otherview->post_title.'</a></center></div>');
					$slider = true;
				}
			}
